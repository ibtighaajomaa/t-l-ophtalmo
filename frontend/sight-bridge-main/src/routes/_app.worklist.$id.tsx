...
function ExamDetail() {
  ...
  const qualityResults = exam.imageQualityResults ?? [];
  const acceptableImageCount = qualityResults.filter(
    (result) => result.category !== "bad" && result.score >= 40,
  ).length;
  const isQualityPoor = qualityResults.length > 0
    ? acceptableImageCount === 0
    : exam.qualityCategory === "bad" || (exam.qualityScore != null && exam.qualityScore < 40);
  const shouldShowAiPanel = aiPanelOpen || (showViewer && hasStudy && !isQualityPoor);
      ...
      <div className="flex-1 p-6 space-y-5">
        ...
        <div className="rounded-xl border border-slate-700 bg-[#0d1117] overflow-hidden shadow-lg">
          ...
          <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-800 bg-[#0a0f1a]">
            <button
              onClick={() => !isQualityPoor && setAiPanelOpen((v) => !v)}
              disabled={isQualityPoor}
            ...
            >
              ...
              {isQualityPoor ? "IA indisponible" : aiPanelOpen ? "AI Analysis ✓" : "AI Analysis"}
            </button>
            <button
          ...
          <div className="space-y-4">
            {shouldShowAiPanel ? (
              <AIPanel
                ...
                qualityCategory={exam.qualityCategory}
                qualityScore={exam.qualityScore}
                imageQualityResults={exam.imageQualityResults}
              />
            ) : (
              <>
                {/* Patient card */}
