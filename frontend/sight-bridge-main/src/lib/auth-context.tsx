...
const SEED_USAGE: UsageEvent[] = [
  ...
  },
  {
    ...
    userName: "Leïla Hadj",
    role: "Medecin",
    at: "2026-06-10T17:30:00Z",
    action: "logout",
  },
  {
    id: "ev-3",
    userId: "u-res",
    userName: "Yanis Mokrane",
    role: "Resident",
    at: "2026-06-11T07:50:00Z",
    action: "login",
  },
  {
    id: "ev-4",
...
interface AuthContextValue {
  ...
  login: (
    email: string,
    password: string,
  ) => Promise<{ ok: boolean; requirePasswordReset?: boolean; error?: string }>;
  logout: () => void;
    ...
    () => ({
      ...
      login: async (email, password) => {
        try {
          ...
          if (response.ok) {
            ...
            if (
              errData.error_code === "invalid_grant" &&
              errData.error?.includes("Account is not fully set up")
            ) {
              return {
                ok: false,
      ...
      createUser: async (data) => {
        try {
          ...
          const token =
            localStorage.getItem("teleoph.token") || sessionStorage.getItem("teleoph.token");
  ...
  if (!initKeycloak) {
    return (
      <div className="flex min-h-screen items-center justify-center text-slate-600">
        Chargement de la sécurité Keycloak...
      </div>
    );
  }
