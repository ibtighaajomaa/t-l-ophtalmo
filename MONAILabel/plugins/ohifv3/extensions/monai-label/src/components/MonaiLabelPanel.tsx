...
export default class MonaiLabelPanel extends Component {
  ...
  serverURI = (typeof window !== 'undefined' ? window.location.origin : 'http://127.0.0.1') + '/monai/';
  ...
  segmentColor(label) {
    const normalizedLabel = String(label || '')
      ...
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/[\s-]+/g, '_');
    const deepLabLesionColors = {
      Microaneurysms: [255, 50, 50, 255],
      microanevrismes: [255, 50, 50, 255],
      Hemorrhages: [50, 50, 255, 255],
      hemorragies: [50, 50, 255, 255],
      hard_exudates: [160, 160, 160, 255],
      soft_exudates: [0, 255, 0, 255],
      cotton_wool_spots: [0, 255, 0, 255],
    };
    if (deepLabLesionColors[normalizedLabel]) {
      return deepLabLesionColors[normalizedLabel];
    }
  ...
  onInfo = async (serverURI) => {
    ...
    if (!volumeLoadObject) {
      ...
      setTimeout(applySegmentColors, 1000);
    } else {
      applySegmentColors();
    }
